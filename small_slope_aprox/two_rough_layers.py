...
def Qmas(x1, x2, y1, y2, ki, ep0, ep1):
    ...
    r00 = f*(X*Y*(1+rv)+(rv-1)*alpha(ki, y1, y2, ep0)
             * alpha(ki, x1, x2, ep1)*x_dot_y)/D
...
def T10p_U0p(ux, uy, qx, qy, ki, ep0, ep1, ep2, d):
    ...
    z22 = 2*alpha(ki, ux, uy, ep1)/(alpha(ki, ux, uy, ep0) +
                                    alpha(ki, ux, uy, ep1))*1/(rh10*rh21*f+1)
...
def Qmas_mas(ux, uy, qx, qy, ki, ep0, ep1, ep2, d):
    ...
    m01 = (ep1-ep0)/(a_u*b_q)*(-np.sqrt(ep0)*ki*alpha(ki, ux, uy, ep1)*Fmenos_v(ux, uy,
                                                                                qx, qy, ki, ep0, ep1, ep2, d)*Fmas_h(qx, qy, qx, qy, ki, ep0, ep1, ep2, d)*u_X_q)

    m10 = (ep1-ep0)/(a_q*b_u)*(-np.sqrt(ep0)*ki*alpha(ki, qx, qy, ep1)*Fmas_h(ux, uy,
                                                                              qx, qy, ki, ep0, ep1, ep2, d)*Fmenos_v(qx, qy, qx, qy, ki, ep0, ep1, ep2, d)*u_X_q)

    m11 = (ep1-ep0)/(b_u*b_q)*ki**2*Fmas_h(ux, uy, qx, qy, ki, ep0, ep1,
                                           ep2, d)*Fmas_h(qx, qy, qx, qy, ki, ep0, ep1, ep2, d)*u_dot_q
...
def suma_O1(ki, ths, phs, thi, phi, ep1, ep2, s1, l1, s2, l2, d):
    ...
    out_ub = f*np.abs(X_u)**2*W(ksx-kix, ksy-kiy, s1, l1) + \
        f*np.abs(X_b)**2*W(ksx-kix, ksy-kiy, s2, l2)
